...
function load(){const s=localStorage.getItem("with_welfare_v2"); if(s) return JSON.parse(s); localStorage.setItem("with_welfare_v2",JSON.stringify(seed)); return JSON.parse(JSON.stringify(seed));}
function save(){localStorage.setItem("with_welfare_v2",JSON.stringify(state));}
...
function loginUser(e){e.preventDefault();const f=new FormData(e.target);const u=state.users.find(x=>x.name===f.get("name")&&x.phone===f.get("phone")&&x.password===f.get("password"));if(!u)return toast("로그인 정보가 일치하지 않습니다.");if(u.status!=="가입승인")return toast("관리자 가입 승인 후 로그인 가능합니다.");session={id:u.id,role:"user"};localStorage.setItem("with_session_v2",JSON.stringify(session));render();}
function loginAdmin(e){e.preventDefault();const f=new FormData(e.target);if(f.get("id")!==adminAccount.loginId||f.get("password")!==adminAccount.password)return toast("관리자 ID 또는 비밀번호가 일치하지 않습니다.");session={id:"admin",role:"admin"};localStorage.setItem("with_session_v2",JSON.stringify(session));render();}

// async async 오타 수정 완료
