...
// [버그 수정] 저장 데이터가 비어있지 않다면 seed로 덮어쓰는 버그를 차단합니다.
function load() {
  ...
  if (s) {
    try {
      ...
      console.error("데이터 파싱 오류로 인해 초기화 데이터를 복구합니다.", e);
    }
  }
...
function signup(e) {
  ...
  save(); // 가입 직후 로컬스토리지에 안정적으로 낙인 처리
...
// [버그 수정] 승인 처리와 동시에 확실하게 브라우저 로컬스토리지에 동기화 저장을 수행합니다.
