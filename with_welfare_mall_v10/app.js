...
// 1. Supabase 접속 환경 설정 변수
const API_KEY="sb_publishable_trTC1dyramnwWtd4XV9eqw_zRWEAY47";
...
const seed = {
  ...
  settings: {
    ...
    annualNightLimit:10,
    condolenceEmail:"welfare@withmedical.co.kr",
    vacationEmail:"vacation@withmedical.co.kr"
  },
  usePolicies:[
...
// 오프라인/에러 발생 시 작동할 폴백(Fallback) 로드 함수
function loadLocalBackup(){const s=localStorage.getItem("with_welfare_v5"); if(s) return JSON.parse(s); localStorage.setItem("with_welfare_v5",JSON.stringify(seed)); return JSON.parse(JSON.stringify(seed));}

// Supabase 실시간 업로드 연동을 포함한 save 함수
async function save(){
  if(!state) return;
  // 1. 브라우저 로컬 스토리지 실시간 백업
  localStorage.setItem("with_welfare_v5", JSON.stringify(state));
  
  // 2. Supabase 클라우드 데이터베이스 동기화
  if (_supabase) {
    await _supabase
      .from('welfare_state')
      .update({ data: state })
...
function stayAdminGroup(){
  return `<div class="group-box">
    ...
    <div class="group-section"><div class="subtle-title"><h3>숙소 할인 조건 설정</h3></div>${policyAdmin()}</div>
  </div>`;
}
...
async function startApp() {
  try {
    ...
    // Supabase의 welfare_state 테이블에서 싱글 레코드(id: 1) 데이터를 원격 로드
    ...
    if (data && data.data) {
      // 클라우드 데이터베이스가 존재하면 가져와서 동기화
      state = data.data;
    } else {
      // 테이블이 완전히 비어있을 경우, 최초 초기값(seed)을 데이터베이스에 적재
      state = JSON.parse(JSON.stringify(seed));
      await _supabase.from('welfare_state').insert([{ id: 1, data: state }]);
    }
    
    migrate();
    render();
  } catch (e) {
    console.error("Supabase 로드 실패, 로컬 백업 모드로 복구 작동:", e);
    // 서버 오류 시 사이트가 뻗거나 안 열리지 않도록 로컬스토리지를 백업하여 복구
